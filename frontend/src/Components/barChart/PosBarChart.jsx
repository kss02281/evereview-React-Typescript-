...
function PosBarChart() {
  ...
  return (
    <ResponsiveContainer width="90%" height="90%">
      <BarChart data={analysisData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
        <XAxis dataKey="id" interval={0} angle={0} dx={0} dy={10} />
        <Tooltip content={<CustomTooltip />} />
        <Bar dataKey="box" barSize={12} radius={[25, 25, 25, 25]} stackId="a" fill="#fff" />
        <Bar dataKey="댓글수" barSize={12} radius={[25, 25, 25, 25]} stackId="a" fill="#8884D8" />
        <Bar dataKey="box" barSize={12} radius={[25, 25, 25, 25]} stackId="a" fill="#fff" />
        <Bar dataKey="좋아요수" barSize={12} radius={[25, 25, 25, 25]} stackId="a" fill="#B4B0FF" />
      </BarChart>
    </ResponsiveContainer>
  );
}
