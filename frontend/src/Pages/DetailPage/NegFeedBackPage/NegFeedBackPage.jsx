...
import { nowAllDateTenArray, nowAllTenArray, nowAnalysis, nowClusterData, nowNegDateFiveArray, nowNegFiveArray } from "store/modules/analysis";
...
function NegFeedBackPage() {
  ...
  const nowDateNegFive = useSelector(nowNegDateFiveArray);
  ...
  const getUserInfo = () => {
    ...
    setClusterData([obj]);
    dispatch(actions.setNegCluster(clusterData));
  } else {
    for (let i = 15; i < 20; i++) {
    ...
    setClusterData([obj]);
    dispatch(actions.setNegDateCluster(clusterData));
  }
  };
  ...
  async function getFeedBackList() {
    ...
    console.log(nowNegFive);
  } else {
    setThisData(nowDateNegFive);
    setIsSelectedCommentArray(Array.from({ length: nowDateNegFive.length }, (v, i) => false));
    console.log(nowDateNegFive);
  }
  }
