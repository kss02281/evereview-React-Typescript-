...
import { nowPosFiveArray, nowPosDateFiveArray, nowAnalysis, nowClusterData } from "store/modules/analysis";
...
const nowPosFive = useSelector(nowPosFiveArray);
const nowDatePosFive = useSelector(nowPosDateFiveArray);
...
const isSelectedVideoList = useSelector(nowSelectedVideoList);
  ...
  const getUserInfo = () => {
    ...
    setClusterData([obj]);
    dispatch(actions.setPosCluster(clusterData));
  } else {
    for (let i = 10; i < 15; i++) {
    ...
    setClusterData([obj]);
    dispatch(actions.setPosDateCluster(clusterData));
  }
  };

  async function getFeedBackList() {
    if ( isCategorySelect.category === "영상별 분석" ){
    setThisData(nowPosFive);
    setIsSelectedCommentArray(Array.from({ length: nowPosFive.length }, (v, i) => false));
    console.log(nowPosFive);
  } else {
    setThisData(nowDatePosFive);
    setIsSelectedCommentArray(Array.from({ length: nowDatePosFive?.length }, (v, i) => false));
    console.log(nowDatePosFive);
  }
  }
      ...
      <div className={cx("feedBackContainer")}>
        ...
        <div className={cx("feedBackWrap")}>
          ...
          <div className={cx("feedBackContentWrap")}>
            ...
            <Link className={cx("posFeedbackSelected")} to={ROUTES.POSFEEDBACK} onClick={setVideo}>
              긍정 피드백
            </Link>
                        ...
                        {thisData?.map((data, i) => {
                          return (
                            <div className={cx("chartWrap")}>
                              <div className={cx("chartLeft")}>{i + 1}.</div>
                              <div className={cx("chartRight")}>
                              {data.top_comment?.text_display?.replace(/(<([^>]+)>)/gi, "").replace(/\n/, "").length > 15
                                  ? data?.top_comment?.text_display?.replace(/(<([^>]+)>)/gi, "")
                                    .replace(/\n/, "")
                                    .substring(0, 15) + "..."
                                  : data.top_comment?.text_display?.replace(/(<([^>]+)>)/gi, "").replace(/\n/, "")}
                              </div>
